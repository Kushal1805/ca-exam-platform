...
async function extractQuestionsFromKey(pdfPath, outputJson) {
    try {
        ...
        const prompt = `
You are analyzing an official CA Exam Answer Key document.
...
Since this is an Answer Key, the question text itself might be missing. You must cleverly reconstruct the question intent, or explicitly write "Constructed Question: [Topic Area]" based on the answer.
...
async function main() {
    await extractQuestionsFromKey('public/Papers/Answer keys/Business Economics Answer Key.pdf', 'Economics.json');
    await extractQuestionsFromKey('public/Papers/Answer keys/Quantitative Aptitude Answer Key.pdf', 'Maths.json');
}
